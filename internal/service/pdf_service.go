...
func (s *PDFService) appendPassportDetailsSection(pdf *gofpdf.Fpdf, passportData *domain.PassportData) {
	...
	fields := []struct {
		...
		{label: "Date of Birth", value: formatPassportDatePointer(passportData.DateOfBirth)},
		...
		{label: "Expiry Date", value: formatPassportDatePointer(passportData.ExpiryDate), alert: isPassportExpiringSoonPointer(passportData.ExpiryDate)},
	}
...
func (s *PDFService) drawApplicationProfileSheet(
	...
	ageValue := formatIntPointer(candidate.Age)
	if passportData != nil {
		if derivedAge := passportData.Age(); derivedAge > 0 {
			ageValue = fmt.Sprintf("%d YRS", derivedAge)
		}
	}
	applicantRows := []pdfFormRow{
		{Label: "Nationality", Value: compactValue(strings.ToUpper(passportValue(passportData, func(data *domain.PassportData) string { return data.Nationality })), "ETHIOPIAN")},
		{Label: "Date Of Birth", Value: passportDatePointerValue(passportData, func(data *domain.PassportData) *time.Time { return data.DateOfBirth })},
		{Label: "Age", Value: ageValue},
		{Label: "Place Of Birth", Value: compactValue(strings.ToUpper(passportValue(passportData, func(data *domain.PassportData) string { return data.PlaceOfBirth })), "N/A")},
		{Label: "Gender", Value: compactValue(strings.ToUpper(passportValue(passportData, func(data *domain.PassportData) string { return data.Gender })), "N/A")},
		{Label: "Country Code", Value: compactValue(strings.ToUpper(passportValue(passportData, func(data *domain.PassportData) string { return data.CountryCode })), "ETH")},
		{Label: "Issuing Auth.", Value: compactValue(strings.ToUpper(passportValue(passportData, func(data *domain.PassportData) string { return data.IssuingAuthority })), "N/A")},
	...
	passportRows := []pdfFormRow{
		...
		{Label: "Expiry Date", Value: passportDatePointerValue(passportData, func(data *domain.PassportData) *time.Time { return data.ExpiryDate })},
		{Label: "Remaining Year", Value: passportRemainingYears(passportData)},
	}
	s.drawFormTable(pdf, rightX, detailY, rightW, "Passport Detail", passportRows, 22)
	...
	s.drawFormTable(pdf, rightX, detailY+42, rightW, "Language Known", langRows, 18)
	...
	s.drawFormTable(pdf, rightX, detailY+78, rightW, "Work Experience", experienceRows, 22)

	expY := detailY + 86
	s.drawSplitHistorySection(pdf, 15, expY, 111, "Experienced Abroad", []string{"Country", "How Long"}, [][2]string{
		{"NO", "-"},
		{"NO", "-"},
		{"NO", "-"},
	})
...
func passportRemainingYears(passportData *domain.PassportData) string {
	if passportData == nil || passportData.ExpiryDate == nil || passportData.ExpiryDate.IsZero() {
		return "N/A"
	}
	now := time.Now().UTC()
...
func isPassportExpiringSoonPointer(expiryDate *time.Time) bool {
