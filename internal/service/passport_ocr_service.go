...
func NewPassportOCRService(
	...
	tessdataPath := ""
	if cfg != nil {
		tessdataPath = strings.TrimSpace(cfg.TessDataPath)
	...
	return &PassportOCRService{
		...
		ocrProcessor:        ocr.NewOCRProcessor(ocrLanguage, tessdataPath),
	}, nil
}
...
func (s *PassportOCRService) ParseAndStore(candidateID, requestedBy string, file io.Reader, fileName string) (*domain.PassportData, error) {
	...
	passportData := buildPassportDataFromOCR(candidateID, result)
...
func (s *PassportOCRService) ParsePreview(file io.Reader, fileName string) (*domain.PassportData, error) {
	...
	return buildPassportDataFromOCR("", result), nil
}

func buildPassportDataFromOCR(candidateID string, result *ocr.PassportExtractResult) *domain.PassportData {
	...
	if !result.DateOfBirth.IsZero() {
		dob := result.DateOfBirth.UTC()
		passportData.DateOfBirth = &dob
	}
	if !result.DateOfExpiry.IsZero() {
		expiry := result.DateOfExpiry.UTC()
		passportData.ExpiryDate = &expiry
	}
