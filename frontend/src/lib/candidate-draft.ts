...
function runFileTransaction<T>(
  ...
  callback: (store: IDBObjectStore, setResult: (value: T) => void) => void,
): Promise<T> {
...
export async function saveCandidateDraftFile(
  ...
  await runFileTransaction<void>("readwrite", (store) => {
...
export async function loadCandidateDraftFile(
  ...
  return runFileTransaction<File | null>("readonly", (store, setResult) => {
    const request = store.get(getDraftFileKey(documentType));
    request.onsuccess = () => {
      setResult((request.result as File | undefined) ?? null);
    };
    request.onerror = () => {
      setResult(null);
    };
